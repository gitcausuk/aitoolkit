(function () {
  ...
  function renderReport(report) {
    ...
    $('#markingPanel').innerHTML =
      ...
      '<h3>Illustrative marking criteria and starting weights</h3><p>These weights follow a fixed editable template and total 100%. They are not calculated from evidence or validated for your local context.</p><table class="report-table"><thead><tr><th>Criterion</th><th>Weight</th></tr></thead><tbody>' + report.criteria.map(item => '<tr><td>' + escapeHTML(item.label) + '</td><td>' + item.weight + '%</td></tr>').join('') + '</tbody></table>' +
  ...
  function reportWordHTML(report) {
    ...
    return '<!DOCTYPE html><html><head><meta charset="utf-8"><title>' + escapeHTML(data.assessmentTitle || 'Assessment redesign') + '</title>' +
      ...
      '<h2>Illustrative marking criteria and starting weights</h2><p>Fixed editable template; not a calculated or validated weighting model.</p><table><tr><th>Criterion</th><th>Weight</th></tr>' + rows + '</table>' +
  ...
  function saveInputs() {
    const data = getData();
    const exportData = { exportedAt: new Date().toISOString(), app: 'AI Designer 1.2.0', inputs: data };
