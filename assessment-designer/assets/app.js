(function () {
  ...
  function validateStep(step, showMessage) {
    ...
    $$('[required]', panel).forEach(input => input.removeAttribute('aria-invalid'));
  ...
  function updateProgress() {
    ...
    window.setTimeout(() => {
      const heading = $('.form-step.active h2');
      if (heading) {
        heading.tabIndex = -1;
  ...
  function metricCard(label, indicator) {
    return '<div class="metric-card"><span class="metric-label">' + escapeHTML(label) + '</span><strong class="metric-value qualitative">' + escapeHTML(indicator.label) + '</strong><span class="metric-note">' + escapeHTML(indicator.note) + '</span></div>';
  }
  ...
  function renderReport(report) {
    ...
    $('#overviewPanel').innerHTML =
      ...
      '<div class="callout success"><strong>' + (data.permission === 'unknown' ? 'Draft Toolkit permission suggestion: ' : 'Selected Toolkit permission: ') + escapeHTML(report.permissionInfo.title) + '</strong><p>' + escapeHTML(report.permissionInfo.rationale) + ' ' + escapeHTML(report.permissionInfo.localMap) + '</p><details><summary>Why this result?</summary><p>' + escapeHTML(report.permissionReason) + '</p></details></div>' +
    ...
    $('#aiPromptPanel').innerHTML =
      ...
      '<div class="result-actions" style="justify-content:flex-start;margin-bottom:12px"><button type="button" class="secondary-button compact" data-copy="ai-prompt">Copy prompt</button><a class="primary-button compact" href="' + ADVISER_URL + '" target="_blank" rel="noopener noreferrer" aria-label="Open AI Adviser (opens in a new tab)" style="text-decoration:none">Open AI Adviser</a></div>' +
    ...
    window.setTimeout(() => {
      const heading = $('.results-header h1');
      if (heading) {
        heading.tabIndex = -1;
        heading.focus({ preventScroll: true });
      }
